...
class _UserProfileScreenState extends State<UserProfileScreen> {
  ...
  final int userId = 1; // Replace with actual user ID from auth
  @override
  void initState() {
    super.initState();
    getProfile();
  ...
  Future<void> getFavoriteBooks() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        debugPrint('Favorite books: $data');
        setState(() {
          favoriteBooks = data;
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text('Error fetching favorites: ${response.body}')),
          );
        }
      }
    } catch (e) {
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      _buildSection(
                          "What You Borrow", ["book3.png", "book4.png"]),
                      const SizedBox(height: 15),
                    ],
                  ),
                ),
